//
// Created by Brett Hartman on 2019-01-03.
//
...
public:
  explicit game(const player &player) : number_generator_(1, 10) {
    player_ = player;
  }
  ...
  int get_bet_amount() {
    ...
    std::cout << "Enter bet amount (1 - " << std::to_string(player_.total_money)
              << "): ";
    std::cin >> wager;
  ...
  wager_result play_round() {
    const int wager = get_bet_amount();
    const int guess = get_guess();
    const int number = number_generator_.generate_random_number();
    ...
    if (number == guess) {
      ...
      result.money_change_amount = wager * -1;
    }
  ...
  bool game_over() { return player_.total_money <= 0; }
};
...
int main() {
  ...
  do {
    const wager_result result = game.play_round();
